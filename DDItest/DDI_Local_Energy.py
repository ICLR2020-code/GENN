...
from torch_geometric.nn.models.model_DDI_Energy_Net import DDI_Energy_Pooling, DDI_LocalEnergy_Net
import warnings
...
model_save_path = osp.join(osp.dirname(osp.realpath(
    __file__)), '../saved_models/', 'LocalEnergy_dataset_{}_seed_{}_ratio_{}'.format(args.data_prefix, args.seed, args.data_ratio) + str('_no_ce_loss' if args.no_ce_loss else ''))
...
ddi_energy_model = DDI_LocalEnergy_Net(input_feature_dim=dataset.x.size()[1], num_types=num_types, dim=dim).to(
...
def train_max(epoch):  # max phi
    ...
    h2 = ddi_model.encode(
        data.x, data.edge_index, data.edge_attr)
    pred2 = ddi_model.decoder2(h2, dataset.edge_index)
    energy_psi = ddi_energy_model(dataset.x, dataset.edge_index, pred2)
